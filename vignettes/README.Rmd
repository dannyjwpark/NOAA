...
# Cleaning and plotting NOAA earthquake data
...
```
earthquakes_data_raw <- load_data()
```

After we have this raw data the process of cleaning it is completed with eq_clean_data():
```
eq_clean_data <- function(earthquakes_data_raw){
  earthquakes_data <- earthquakes_data_raw
  earthquakes_data <- earthquakes_data %>% dplyr::filter(YEAR >= 0) %>%
...
```
earthquakes_data <- df_earthquakes %>% filter(COUNTRY %in% c("CHINA", "USA"), YEAR > 2000)
